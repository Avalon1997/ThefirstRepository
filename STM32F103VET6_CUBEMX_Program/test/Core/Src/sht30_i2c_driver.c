 ...
 */
 ...
 static uint8_t SHT30_Send_CMD(SHT30_CMD cmd)
 {
     ...
     cmd_buffer[0] = cmd >> 8;
     cmd_buffer[1] = cmd;
     return HAL_I2C_Master_Transmit(&hi2c1,SHT30_ADDR_WRITE,(uint8_t*) cmd_buffer,2,20000);
 }
...
void Measure_TR(void)
{
    ...
    {   
        ...
        {
            ErrorOne = SHT30_Dat_To_Float(I2CRXBuffer,&Temperature,&Humidity);
            if (!ErrorOne)
            {
                ...
                printf("Temperature:%.2fC,Humidity:%.2f%%",Temperature,Humidity);
