...
int main(void)
{
  ...
  while (1)
  {
    ...
    //串口中断接收函数测试
    // HAL_UART_Receive_IT(&huart1,rebuffer,6);
    // if (rebuffer[0] == 0x11 && rebuffer[1] == 0x22 && rebuffer[2] == 0x33 && rebuffer[3] == 0x44 && rebuffer[4] == 0x55 && rebuffer[5] == 0x66)
    // {
    //   HAL_UART_Transmit(&huart1,dataone,sizeof(dataone),0xFFFF);
